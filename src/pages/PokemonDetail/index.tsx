...
export default function PokemonDetailScreen() {
  ...
  async function handleToggleFavorite() {
    if (!pokemon) return;
    const summary: Omit<FavoritePokemon, "savedAt"> = {
      id: pokemon.id,
      name: pokemon.name,
      imageUrl:
  ...
  async function handleSharePokemon() {
    ...
    let message = `Olha esse bicho estranho aí: ${pokemon.name} (#${String(pokemon.id).padStart(3, "0")})`;
    ...
    message += `\nStats base: ${pokemon.stats
      .map((s) => `${s.stat.name.toUpperCase()}: ${s.base_stat}`)
      .join(", ")}`;
    message += `\nConfira mais detalhes sobre ele aqui: ${pokeApiUrl}`;
  ...
  async function saveLastViewed() {
    ...
    await saveLastViewedPokemon(summary);
  }
        ...
        <View style={styles.typeContainer}>
          {pokemon.types.map(({ type }) => (
            <View
              ...
              style={[
                styles.typeBadge,
                { backgroundColor: TYPE_COLORS[type.name] ?? "#A8A8A8" },
              ]}
            >
